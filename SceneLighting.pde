class PLight extends Light {
  ...
  Vec getDirection(Vec hit) {
    return normalizeV(subV(origin, hit));
  }
}
...
class diskLight extends Light {
  public Vec normal;
  public float radius;
  diskLight(Vec origin) { 
    super(origin, "disk_light");
  }
  diskLight(Vec origin, float radius, Vec normal, Vec light_color) { 
    super(origin, "disk_light");
    this.radius = radius; 
    this.normal = normal; 
    this.light_color = light_color;
  ...
  Vec getDirection(Vec hit) {
    ...
    if (abs(dotV(V)) < .00005 ) {
      V = crossV(normal, V(0, 1, 0)).normalize();
    }
    if (abs(dotV(V)) < .00005 ) {
      V = crossV(normal, V(0, 0, 1)).normalize();
    }
